...
public abstract class TxtReaderBaseView extends View implements GestureDetector.OnGestureListener {
    ...
    protected static int PageChangeMinMoveDistance = 50;//页面切换需要的最小滑动距离
    ...
    protected TxtFileMsg.TxtChar FirstSelectedChar = null;//第一个选中的字符
    protected TxtFileMsg.TxtChar LastSelectedChar = null;//最后一个选中的字符
    ...
    private TouchListener mTounchListener;
        ...
        ELogger.log("onLongPress", "CurrentMode:" + CurrentMode);
            if (CurrentMode == Mode.SelectMoveBack) {
                ...
                if (CanMoveBack(x, y)) {
                    ...
                    TxtFileMsg.TxtChar moveToChar = findCharByPosition(x, y);
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        mTouch.x = e.getX();
        mTouch.y = e.getY();
        if ((getWidth()/3) < mDown.x && mDown.x < (getWidth() * 2 / 3) && getHeight()/5 < mDown.y && mDown.y < getHeight()*4/5){
            if (mTounchListener != null){
                mTounchListener.center();
                return true;
            }
        } else if(getWidth()/3 > mDown.x){
            if(!isFirstPage()){
            tryDoPagePre();
            startPagePreAnimation();
            }
            return true;
        } else //if (getWidth()*2/3 < mDown.x)//
        {
            if(!isLastPage()){
            tryDoPageNext();
            startPageNextAnimation();
    ...
    public boolean onScroll(MotionEvent motionEvent, MotionEvent motionEvent1, float v, float v1) {
        if (is) {
            mDown.x = motionEvent.getX();
            mTouch.x = motionEvent1.getX();
            onPageMove(motionEvent1);
            return true;
        }else {
            return false;
        }
...
//        }
    ...
    private void onPressSelectText(MotionEvent e) {
        TxtFileMsg.TxtChar selectedChar = findCharByPosition(e.getX(), e.getY());
        if (selectedChar != null) {
            ...
            cm.setPrimaryClip(ClipData.newPlainText("text", selectedChar.getValueStr()));
    ...
    private void setLeftSlider(TxtFileMsg.TxtChar FirstSelectedChar) {
    ...
    private void setRightSlider(TxtFileMsg.TxtChar LastSelectedChar) {
     ...
     */
    private TxtFileMsg.TxtChar findCharByPosition(float positionX, float positionY) {
        ...
        if (page != null && page.HasData()) {
            ...
            for (ITxtLine line : lines) {
                List<TxtFileMsg.TxtChar> chars = line.getTxtChars();
                if (chars != null && chars.size() > 0) {
                    for (TxtFileMsg.TxtChar c : chars) {
    ...
    protected synchronized void checkSelectedText() {
        ...
        for (ITxtLine l : lines) {
            ...
            for (TxtFileMsg.TxtChar c : l.getTxtChars()) {
                if (!Started) {
    ...
    private void initReaderContext() {
        PageChangeMinMoveDistance = getWidth() / 25;
        PageParam param = new PageParam();
        param.PageWidth = getResources().getDisplayMetrics().widthPixels > getWidth() ? getResources().getDisplayMetrics().widthPixels : getWidth();
        param.PageHeight = getResources().getDisplayMetrics().heightPixels > getHeight() ? getResources().getDisplayMetrics().heightPixels : getHeight();
        ...
        private void getPageNextData() {
            ...
            if (firstPage != null && nextFirstPage != null) {
                if (firstPage.HasData() && nextFirstPage.HasData()) {
                    ...
                    TxtFileMsg.TxtChar midF = firstPage.getFirstChar();
                    TxtFileMsg.TxtChar midL = firstPage.getLastChar();

                    TxtFileMsg.TxtChar nextF = nextFirstPage.getFirstChar();
                    TxtFileMsg.TxtChar nextL = nextFirstPage.getLastChar();
            ...
            if (midPage != null && nextMidPage != null) {
                if (midPage.HasData() && nextMidPage.HasData()) {
                    TxtFileMsg.TxtChar midF = midPage.getFirstChar();
                    TxtFileMsg.TxtChar midL = midPage.getLastChar();

                    TxtFileMsg.TxtChar nextF = midPage.getFirstChar();
                    TxtFileMsg.TxtChar nextL = midPage.getLastChar();
    ...
    private class PagePreTask implements ITxtTask {
        ...
        private void getPagePreData() {
            ...
            if (nextMayMidPage != null && nextMayMidPage.HasData()) {
                if (nextMayMidPage.getLineNum() == lineNum) {
                    //之前的FirstPage是满页的，直接获取
            ...
            if (nextPage != null && nextPage.HasData() && nextMayLastPage != null && nextMayLastPage.HasData()) {
                ...
                TxtFileMsg.TxtChar midF = nextPage.getFirstChar();
                TxtFileMsg.TxtChar midL = nextPage.getLastChar();

                TxtFileMsg.TxtChar nextF = nextMayLastPage.getFirstChar();
                TxtFileMsg.TxtChar nextL = nextMayLastPage.getLastChar();
            ...
            if (midPage != null && midPage.HasData() && nextMayMidPage != null && nextMayMidPage.HasData()) {
                TxtFileMsg.TxtChar midF = midPage.getFirstChar();
                TxtFileMsg.TxtChar midL = midPage.getLastChar();
                TxtFileMsg.TxtChar nextF = nextMayMidPage.getFirstChar();
                TxtFileMsg.TxtChar nextL = nextMayMidPage.getLastChar();
    ...
    protected void onPageProgress(IPage page) {
        if (pageChangeListener != null) {
            float progress = 0;
            if (page != null && page.HasData()) {
                TxtFileMsg.TxtChar lastChar = page.getLastChar();
    ...
    public void setTouchListener(TouchListener mTounchListener){
        this.mTounchListener = mTounchListener;
    }
