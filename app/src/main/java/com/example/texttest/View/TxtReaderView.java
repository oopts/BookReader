...
public class TxtReaderView extends TxtReaderBaseView {
    ...
    public int getTxtColor(){
        return readerContext.getTxtConfig().getTextColor(getContext());
    }
    ...
    public Boolean jumpToNextChapter() {
        ...
        if (getCurrentChapter().getIndex() == 1){
            nextChapter = chapters.get(index + 2);
            loadFromProgress(nextChapter.getStartParagraphIndex(), 0);
        }
        return true;
    }
    ...
    public void saveProgress() {
        ...
        if (currentPage != null && currentPage.HasData()) {
            TxtFileMsg.TxtChar firstChar = currentPage.getFirstChar();
